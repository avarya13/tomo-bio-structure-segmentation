...
class ResidualUnetSmall(nn.Module):
    """
    ...
    def __init__(self, input_channels, num_classes, channels_aggr='conv'):
        ...
        self.bottle_conv = nn.Conv2d(64, 128, kernel_size=3, padding=1)
        self.bottle_bn = nn.BatchNorm2d(128)
        self.bottle_residual = ResidualBlock(128, 128)
        
        self.d1 = Decoder(128, 64)
        self.d2 = Decoder(64, 32)
        self.d3 = Decoder(32, 16)
    ...
    def forward(self, x):
        ...
        bottle_neck = F.relu(self.bottle_bn(self.bottle_conv(pool_x3)))
        bottle_neck = self.bottle_residual(bottle_neck)

        d1 = self.d1(bottle_neck, skip_x3)

        d2 = self.d2(d1, skip_x2)

        d3 = self.d3(d2, skip_x1)
